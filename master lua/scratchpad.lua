...
function CallPet(aPetIdx)
  local PetID = 883
  if not aPetIdx then aPetIdx = 1;end;
	if aPetIdx == 1 then
	  PetID = 883;
	else                    
...
function FilterList_MyCreateUnit(List)
	...
	for i = #List,1,-1 do--倒循环 不然移除元素时会出问题 比如10个成员的表 第2个发现不需要要删除 删除以后第3个元素就会成为第2个元素 循环因为第2个走过就会走第3个元素 原来的第3个元素变为第2个的就会跳过不检查
		...
		if PtrCreateGuid ~= MyGuid then--Ptr的创建者不是我
			table.remove(List,i)--不是我创建的Ptr就移除
		end;
	end;
...
function CastSpellBuffExists(SpellID,BuffID,SpellUnit,BuffUnit)--放技能放到BUFF存在
	while BuffExists(BuffID,BuffUnit) ~= 1 do
		...
		if Sleep(500) then return true end;
	end
end;
...
function DPS_Exit(target,DPSf_HP)--DPS目标时的退出条件
	...
	if NowTarget == "00000000000000000000000000000000" then return true end;	--目标无 --No goal
	if target ~= NowTarget then return true end;	--目标切换了 --Target switched
	if GetUnitF_HP("target") <= DPSf_HP then return true end;	--目标血量打到规定值了 --The target HP reached the specified value
	return false;
end;
